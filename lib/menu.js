...
export default class {
  ...
  async clean() {
    ...
    if (user && user.teams) {
      user.teams.forEach(team => {
        app.commands.addCommand(`kyso:publish:${team.name}`, Publish(_extends({}, commandProps, {
          team: team.name,
  ...
  async render() {
    ...
    if (user) {
      ...
      if (user.teams) {
        const newGroup = [{ type: 'submenu', submenu: mainMenu.kysoMenu.publishMenu.menu }, { command: 'publish:submenu' }];
